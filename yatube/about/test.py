...
class StaticURLTests(TestCase):
    ...
    def test_static_page(self) -> None:
        """Проверка доступности адреса URL."""
        pages = ("/about/author/", "/about/tech/",)
        for page in pages:
            response = self.guest_client.get(page)
            error_msg = f"Адрес {page} не доступен"
            self.assertEqual(response.status_code, HTTPStatus.OK, error_msg)
    ...
    def test_urls_uses_correct_template(self) -> None:
        ...
        for adress, template in templates_url_names.items():
            with self.subTest(adress=adress):
                ...
                error_msg: str = f"Ошибка: {adress} ожидал шаблон {template}"
